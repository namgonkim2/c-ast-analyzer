...
int main(int argc, char **argv){
  ...
  CXTranslationUnit unit = clang_parseTranslationUnit(
    index, argv[1], nullptr, 0, nullptr, 0, CXTranslationUnit_None); //Parse "file.cpp"
  ...
  clang_visitChildren(
      ...
      {
      // std::cout << "Type '" << clang_getCString(clang_getTypedefName(clang_getCursorType(c))) << "' / ";
      std::cout << "Cursor '" << clang_getCString(clang_getCursorSpelling(c)) << "' / ";
      std::cout << "Kind '" << clang_getCString(clang_getCursorKindSpelling(clang_getCursorKind(c))) << "'\n";
      return CXChildVisit_Recurse;
      },
      nullptr);
}